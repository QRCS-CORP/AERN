...
#define NETWORK_CONVERGE_UPDATE_SEQUENCE 0xFFFFFF1AUL
...
static aern_protocol_errors network_header_validate_dynamic(const aern_network_packet* packetin, aern_network_flags flag, uint64_t sequence)
{
	AERN_ASSERT(packetin != NULL);
	...
	if (packetin != NULL)
	{
		...
		else if (aern_packet_time_valid(packetin) == false)
		{
			merr = aern_protocol_error_message_time_invalid;
		}
		else if (packetin->sequence != sequence)
		{
			merr = aern_protocol_error_packet_unsequenced;
		}
		else if (packetin->flag != flag)
		{
			merr = aern_protocol_error_invalid_request;
		}
		else
...
* Each APS on the network is sent a copy of their topological node database entry.
...
aern_protocol_errors aern_network_converge_request(const aern_network_converge_request_state* state)
{
	...
	/* the ADC loops through MAS and APS nodes in the topology, 
...
aern_protocol_errors network_converge_request_verify(const aern_network_converge_response_state* state, const aern_network_packet* packetin)
{
	...
	if (merr == aern_protocol_error_none)
	{
		/* verify the ADC certificates signature */
		merr = network_message_signed_hash_verify(rnode, packetin, state->rcert);
...
aern_protocol_errors network_fkey_response_verify(aern_network_fkey_request_state* state, const aern_network_packet* packetin)
{
	...
	if (merr == aern_protocol_error_none)
	{
		...
		if (qsc_memutils_are_equal(mtag, packetin->pmessage + AERN_CRYPTO_SYMMETRIC_KEY_SIZE, AERN_CRYPTO_SYMMETRIC_HASH_SIZE) == true)
		{
			...
			qsc_memutils_secure_erase(ckey, sizeof(ckey));
			merr = aern_protocol_error_none;
		}
		else
		{
...
aern_protocol_errors aern_network_fkey_request(aern_network_fkey_request_state* state)
{
	...
	if (state != NULL)
	{
		...
		if (aern_network_connect_to_device(&csock, state->rnode->address, aern_network_designation_aps) == qsc_socket_exception_success)
		{
			...
			qsc_memutils_secure_erase(sbuf, NETWORK_FRAGMENT_FKEY_REQUEST_PACKET_SIZE);
...
aern_protocol_errors aern_network_fkey_response(aern_network_fkey_response_state* state, const aern_network_packet* packetin)
{
	...
	if (state != NULL && packetin != NULL)
	{
		...
		if (merr == aern_protocol_error_none)
		{
			...
			qsc_memutils_secure_erase(sbuf, NETWORK_FRAGMENT_FKEY_RESPONSE_PACKET_SIZE);
...
aern_protocol_errors aern_network_converge_update(aern_network_converge_update_state* state, const aern_network_packet* packetin)
{
	AERN_ASSERT(state != NULL);
	AERN_ASSERT(packetin != NULL);
	...
	aern_protocol_errors merr;
	uint8_t sdata[AERN_PACKET_SUBHEADER_SIZE] = { 0U };

	merr = aern_protocol_error_invalid_request;

	if (state != NULL && packetin != NULL && state->rcert != NULL && state->root != NULL && state->vtopo != NULL)
	{
		merr = network_header_validate(packetin, aern_network_flag_network_converge_update, NETWORK_CONVERGE_UPDATE_SEQUENCE, NETWORK_CONVERGE_UPDATE_MESSAGE_SIZE);

		if (merr == aern_protocol_error_none)
		{
			network_subheader_serialize(sdata, packetin);

			if (qsc_memutils_are_equal(sdata, packetin->pmessage, AERN_PACKET_SUBHEADER_SIZE) == false)
			{
				merr = aern_protocol_error_message_time_invalid;
			}
		}
		...
		if (merr == aern_protocol_error_none)
		{
			if (aern_certificate_signature_hash_verify(packetin->pmessage + AERN_PACKET_SUBHEADER_SIZE + AERN_NETWORK_TOPOLOGY_NODE_SIZE, AERN_CERTIFICATE_SIGNED_HASH_SIZE, packetin->pmessage, AERN_PACKET_SUBHEADER_SIZE + AERN_NETWORK_TOPOLOGY_NODE_SIZE, state->rcert) == false)
			{
				merr = aern_protocol_error_authentication_failure;
			}
		}
		...
		if (merr == aern_protocol_error_none)
		{
			aern_topology_node_deserialize(&node, packetin->pmessage + AERN_PACKET_SUBHEADER_SIZE);
			merr = aern_topology_update(state->vtopo, &node);

			if (merr == aern_protocol_error_node_not_found)
			{
				aern_topology_add(state->vtopo, &node);
				merr = aern_protocol_error_none;
			}
		}
	}

	qsc_memutils_secure_erase(&node, sizeof(aern_topology_node_state));
	qsc_memutils_secure_erase(sdata, sizeof(sdata));

	return merr;
}

aern_protocol_errors aern_network_join_request(aern_network_join_request_state* state)
{
	AERN_ASSERT(state != NULL);
	...
	uint8_t phdr[AERN_PACKET_HEADER_SIZE] = { 0U };
	uint8_t sdata[AERN_PACKET_SUBHEADER_SIZE] = { 0U };
	uint8_t* rbuf;
	uint8_t* sbuf;
	const uint8_t* rhas;
	...
	rhas = NULL;
	rsig = NULL;
	base = AERN_PACKET_SUBHEADER_SIZE + AERN_CERTIFICATE_CHILD_SIZE + AERN_CERTIFICATE_HASH_SIZE + AERN_CERTIFICATE_SIGNED_HASH_SIZE;
	plen = AERN_PACKET_HEADER_SIZE + base;
	rlen = 0U;
	rmsg = 0U;
	slen = 0U;
	siglen = 0U;

	if (state != NULL && state->address != NULL && state->lcert != NULL && state->rcert != NULL && state->root != NULL && state->sigkey != NULL && state->vtopo != NULL)
	{
		sbuf = qsc_memutils_malloc(plen);

		if (sbuf != NULL)
		{
			qsc_memutils_clear(sbuf, plen);
			network_header_create(&reqt, aern_network_flag_register_update_request, NETWORK_JOIN_REQUEST_SEQUENCE, (uint32_t)base);
			reqt.pmessage = sbuf + AERN_PACKET_HEADER_SIZE;
			network_subheader_serialize(reqt.pmessage, &reqt);
			aern_certificate_child_serialize(reqt.pmessage + AERN_PACKET_SUBHEADER_SIZE, state->lcert);
			aern_topology_hash(state->vtopo, chash);
			qsc_memutils_copy(reqt.pmessage + AERN_PACKET_SUBHEADER_SIZE + AERN_CERTIFICATE_CHILD_SIZE, chash, AERN_CERTIFICATE_HASH_SIZE);
			siglen = aern_certificate_message_hash_sign(reqt.pmessage + AERN_PACKET_SUBHEADER_SIZE + AERN_CERTIFICATE_CHILD_SIZE + AERN_CERTIFICATE_HASH_SIZE, state->sigkey, reqt.pmessage, AERN_PACKET_SUBHEADER_SIZE + AERN_CERTIFICATE_CHILD_SIZE + AERN_CERTIFICATE_HASH_SIZE);

			if (siglen == AERN_CERTIFICATE_SIGNED_HASH_SIZE)
			{
				aern_packet_header_serialize(&reqt, sbuf);

				if (aern_network_connect_to_device(&csock, state->address, aern_network_designation_adc) == qsc_socket_exception_success)
				{
					slen = qsc_socket_client_send(&csock, sbuf, plen, qsc_socket_send_flag_none);

					if (slen == plen)
					{
						rlen = qsc_socket_peek(&csock, phdr, AERN_PACKET_HEADER_SIZE);

						if (rlen == AERN_PACKET_HEADER_SIZE)
						{
							aern_packet_header_deserialize(phdr, &resp);

							if (resp.flag == aern_network_flag_system_error_condition)
							{
								rbuf = qsc_memutils_malloc(NETWORK_ERROR_PACKET_SIZE);
								rmsg = NETWORK_ERROR_PACKET_SIZE;
							}
							else if (resp.msglen >= base && resp.msglen <= AERN_MESSAGE_MAX_SIZE)
							{
								rmsg = AERN_PACKET_HEADER_SIZE + resp.msglen;
								rbuf = qsc_memutils_malloc(rmsg);
							}
							else
							{
								merr = aern_protocol_error_receive_failure;
							...
							if (rbuf != NULL)
							{
								rlen = qsc_socket_receive(&csock, rbuf, rmsg, qsc_socket_receive_flag_wait_all);
								...
								if (rlen == rmsg)
								{
									...
									{
										merr = network_header_validate_dynamic(&resp, aern_network_flag_register_update_response, NETWORK_JOIN_RESPONSE_SEQUENCE);
									}

									if (merr == aern_protocol_error_none)
									{
										network_subheader_serialize(sdata, &resp);

										if (qsc_memutils_are_equal(sdata, resp.pmessage, AERN_PACKET_SUBHEADER_SIZE) == false)
										{
											merr = aern_protocol_error_message_time_invalid;
										}
									}
									...
									if (merr == aern_protocol_error_none)
									{
										aern_certificate_child_deserialize(&adc, resp.pmessage + AERN_PACKET_SUBHEADER_SIZE);
										merr = aern_network_certificate_verify(&adc, state->root);
									}

									if (merr == aern_protocol_error_none)
									{
										rsig = resp.pmessage + AERN_PACKET_SUBHEADER_SIZE + AERN_CERTIFICATE_CHILD_SIZE + AERN_CERTIFICATE_HASH_SIZE;

										if (aern_certificate_signature_hash_verify(rsig, AERN_CERTIFICATE_SIGNED_HASH_SIZE, resp.pmessage, AERN_PACKET_SUBHEADER_SIZE + AERN_CERTIFICATE_CHILD_SIZE + AERN_CERTIFICATE_HASH_SIZE, &adc) == false)
										{
											merr = aern_protocol_error_authentication_failure;
										}
									}
									...
									if (merr == aern_protocol_error_none)
									{
										aern_certificate_child_copy(state->rcert, &adc);
										rhas = resp.pmessage + AERN_PACKET_SUBHEADER_SIZE + AERN_CERTIFICATE_CHILD_SIZE;

										if (qsc_memutils_are_equal(rhas, chash, AERN_CERTIFICATE_HASH_SIZE) == false && resp.msglen > base)
										{
											aern_topology_list_dispose(state->vtopo);
											aern_topology_list_initialize(state->vtopo);
											(void)aern_topology_list_update_unpack(state->vtopo, resp.pmessage + base, resp.msglen - base);
											(void)aern_topology_increment_version(state->vtopo);
										}
									}
								}
								else
							...
							else if (merr == aern_protocol_error_invalid_request)
							{
								merr = aern_protocol_error_memory_allocation;
							}
						}
	...
	if (csock.connection_status == qsc_socket_state_connected)
	{
		aern_network_socket_dispose(&csock);
	}
	...
	if (sbuf != NULL)
	{
		qsc_memutils_secure_erase(sbuf, plen);
		qsc_memutils_alloc_free(sbuf);
	}

	if (rbuf != NULL)
	{
		qsc_memutils_secure_erase(rbuf, rmsg);
		qsc_memutils_alloc_free(rbuf);
	}

	qsc_memutils_secure_erase(&adc, sizeof(aern_child_certificate));
	qsc_memutils_secure_erase(chash, sizeof(chash));
	qsc_memutils_secure_erase(phdr, sizeof(phdr));
	qsc_memutils_secure_erase(sdata, sizeof(sdata));

	return merr;
}

aern_protocol_errors aern_network_join_response(aern_network_join_response_state* state, const aern_network_packet* packetin)
{
	AERN_ASSERT(state != NULL);
	AERN_ASSERT(packetin != NULL);

	aern_child_certificate client = { 0 };
	aern_network_packet resp = { 0 };
	aern_protocol_errors merr;
	uint8_t chash[AERN_CERTIFICATE_HASH_SIZE] = { 0U };
	uint8_t sdata[AERN_PACKET_SUBHEADER_SIZE] = { 0U };
	...
	if (state != NULL && packetin != NULL && state->csock != NULL && state->lcert != NULL && state->rcert != NULL && state->root != NULL && state->sigkey != NULL && state->vtopo != NULL)
	{
		merr = network_header_validate_dynamic(packetin, aern_network_flag_register_update_request, NETWORK_JOIN_REQUEST_SEQUENCE);

		if (merr == aern_protocol_error_none && packetin->msglen != base)
		{
			merr = aern_protocol_error_receive_failure;
		}
		...
		if (merr == aern_protocol_error_none)
		{
			aern_certificate_child_deserialize(&client, packetin->pmessage + AERN_PACKET_SUBHEADER_SIZE);
			merr = aern_network_certificate_verify(&client, state->root);
		}

		if (merr == aern_protocol_error_none)
		{
			sig = packetin->pmessage + AERN_PACKET_SUBHEADER_SIZE + AERN_CERTIFICATE_CHILD_SIZE + AERN_CERTIFICATE_HASH_SIZE;

			if (aern_certificate_signature_hash_verify(sig, AERN_CERTIFICATE_SIGNED_HASH_SIZE, packetin->pmessage, AERN_PACKET_SUBHEADER_SIZE + AERN_CERTIFICATE_CHILD_SIZE + AERN_CERTIFICATE_HASH_SIZE, &client) == false)
			{
				merr = aern_protocol_error_authentication_failure;
			}
		}
		...
		if (merr == aern_protocol_error_none)
		{
			aern_certificate_child_copy(state->rcert, &client);
			aern_topology_hash(state->vtopo, chash);
			hash = packetin->pmessage + AERN_PACKET_SUBHEADER_SIZE + AERN_CERTIFICATE_CHILD_SIZE;
			hmatch = qsc_memutils_are_equal(hash, chash, AERN_CERTIFICATE_HASH_SIZE);

			if (hmatch == false)
			{
				apscnt = aern_topology_list_server_count(state->vtopo, aern_network_designation_aps);

				if (apscnt > 0U)
				{
					tlen = apscnt * AERN_NETWORK_TOPOLOGY_NODE_SIZE;
					tbuf = qsc_memutils_malloc(tlen);

					if (tbuf != NULL)
					{
						qsc_memutils_secure_erase(tbuf, tlen);
						(void)aern_topology_list_update_pack(tbuf, state->vtopo, aern_network_designation_aps);
						msglen += tlen;
					}
					else
					{
		...
		if (merr == aern_protocol_error_none)
		{
			pktlen = AERN_PACKET_HEADER_SIZE + msglen;
			rbuf = qsc_memutils_malloc(pktlen);
		...
		if (merr == aern_protocol_error_none)
		{
			qsc_memutils_secure_erase(rbuf, pktlen);
			network_header_create(&resp, aern_network_flag_register_update_response, NETWORK_JOIN_RESPONSE_SEQUENCE, (uint32_t)msglen);
			resp.pmessage = rbuf + AERN_PACKET_HEADER_SIZE;
			network_subheader_serialize(resp.pmessage, &resp);
			aern_certificate_child_serialize(resp.pmessage + AERN_PACKET_SUBHEADER_SIZE, state->lcert);
			qsc_memutils_copy(resp.pmessage + AERN_PACKET_SUBHEADER_SIZE + AERN_CERTIFICATE_CHILD_SIZE, chash, AERN_CERTIFICATE_HASH_SIZE);
			siglen = aern_certificate_message_hash_sign(resp.pmessage + AERN_PACKET_SUBHEADER_SIZE + AERN_CERTIFICATE_CHILD_SIZE + AERN_CERTIFICATE_HASH_SIZE, state->sigkey, resp.pmessage, AERN_PACKET_SUBHEADER_SIZE + AERN_CERTIFICATE_CHILD_SIZE + AERN_CERTIFICATE_HASH_SIZE);

			if (siglen == AERN_CERTIFICATE_SIGNED_HASH_SIZE)
			{
				if (tbuf != NULL && tlen > 0U)
				{
					qsc_memutils_copy(resp.pmessage + base, tbuf, tlen);
				}

				aern_packet_header_serialize(&resp, rbuf);
				sent = qsc_socket_client_send(state->csock, rbuf, pktlen, qsc_socket_send_flag_none);

				if (sent == pktlen)
				{
					aern_topology_child_register(state->vtopo, state->rcert, "");
					(void)aern_topology_increment_version(state->vtopo);
				}
				else
				{
	...
	qsc_memutils_secure_erase(&client, sizeof(aern_child_certificate));
	qsc_memutils_secure_erase(chash, sizeof(chash));
	qsc_memutils_secure_erase(sdata, sizeof(sdata));
...
aern_protocol_errors aern_network_mfk_exchange_request(aern_network_mfk_request_state* state)
{
	...
	if (state != NULL && state->lcert != NULL && state->mfk != NULL &&
		state->rcert != NULL && state->rnode != NULL && state->root != NULL &&
		state->sigkey != NULL)
	{
		if (aern_network_connect_to_device(&csock, state->rnode->address,
			state->rnode->designation) == qsc_socket_exception_success)
		{
			reqt.pmessage = sbuf + AERN_PACKET_HEADER_SIZE;
			merr = network_mfk_request_packet(&reqt, state);

			if (merr == aern_protocol_error_none)
			{
				aern_packet_header_serialize(&reqt, sbuf);
				slen = qsc_socket_send(&csock, sbuf, NETWORK_MFK_REQUEST_PACKET_SIZE,
					qsc_socket_send_flag_none);

				if (slen == NETWORK_MFK_REQUEST_PACKET_SIZE)
				{
					rlen = qsc_socket_receive(&csock, rbuf, sizeof(rbuf),
					...
					if (rlen == NETWORK_MFK_RESPONSE_PACKET_SIZE)
					{
						aern_packet_header_deserialize(rbuf, &resp);
						resp.pmessage = rbuf + AERN_PACKET_HEADER_SIZE;
						estab.pmessage = ebuf + AERN_PACKET_HEADER_SIZE;
						merr = network_mfk_establish_packet(&estab, &resp, state);

						if (merr == aern_protocol_error_none)
						{
							aern_packet_header_serialize(&estab, ebuf);
							slen = qsc_socket_send(&csock, ebuf,
								NETWORK_MFK_ESTABLISH_PACKET_SIZE,
								qsc_socket_send_flag_none);
	...
	qsc_memutils_secure_erase(ebuf, sizeof(ebuf));
	qsc_memutils_secure_erase(rbuf, sizeof(rbuf));
...
aern_protocol_errors aern_network_mfk_exchange_response(aern_network_mfk_response_state* state, const aern_network_packet* packetin)
{
	...
	if (state != NULL && packetin != NULL && state->csock != NULL &&
		state->lcert != NULL && state->mfk != NULL && state->rcert != NULL &&
		state->root != NULL && state->sigkey != NULL)
	{
		resp.pmessage = sbuf + AERN_PACKET_HEADER_SIZE;
		...
		{
			aern_packet_header_serialize(&resp, sbuf);
			slen = qsc_socket_send(state->csock, sbuf,
				NETWORK_MFK_RESPONSE_PACKET_SIZE, qsc_socket_send_flag_none);

			if (slen == NETWORK_MFK_RESPONSE_PACKET_SIZE)
			{
				rlen = qsc_socket_receive(state->csock, rbuf, sizeof(rbuf),
					qsc_socket_receive_flag_wait_all);
	...
	if (merr != aern_protocol_error_none && state != NULL && state->csock != NULL)
	{
		aern_network_send_error(state->csock, merr);
	}
...
aern_protocol_errors aern_network_register_request(aern_network_register_request_state* state)
{
	...
	if (state != NULL)
	{
		if (aern_network_connect_to_device(&csock, state->address, aern_network_designation_adc) == qsc_socket_exception_success)
		{
			...
			/* send the join request to the ADC */
...
aern_protocol_errors network_register_response_packet(aern_network_packet* packetout, aern_network_register_response_state* state, const aern_network_packet* packetin)
{
	...
	if (merr == aern_protocol_error_none)
	{
		...
		if (merr == aern_protocol_error_none && state->rcert->designation != aern_network_designation_aps)
		{
			merr = aern_protocol_error_invalid_request;
		}
...
aern_protocol_errors aern_network_register_response(aern_network_register_response_state* state, const aern_network_packet* packetin)
{
	...
	if (state != NULL && packetin != NULL)
	{
		...
		/* apss are sent only the root-signed ADC certificate */
		resp.pmessage = sbuf + AERN_PACKET_HEADER_SIZE;
			...
			if (merr == aern_protocol_error_none)
			{
				...
				if (pmsg != NULL)
				{
					...
					if (merr == aern_protocol_error_none)
					{
						...
						if (mlen >= sizeof(uint64_t) && ((mlen - sizeof(uint64_t)) % AERN_NETWORK_TOPOLOGY_NODE_SIZE) == 0U)
						{
							uint64_t rver;

							rver = qsc_intutils_le8to64(pnds);

							if (rver != 0U && (state->list->version == 0U || rver >= state->list->version))
							{
								pnds += sizeof(uint64_t);
								mlen -= sizeof(uint64_t);

								if (mlen == 0U || aern_topology_list_update_unpack(state->list, pnds, mlen) == (mlen / AERN_NETWORK_TOPOLOGY_NODE_SIZE))
								{
									state->list->version = rver;
									merr = aern_protocol_error_none;
								}
								else
								{
						...
						else if (mlen == 0U || aern_topology_list_update_unpack(state->list, pnds, mlen) == (mlen / AERN_NETWORK_TOPOLOGY_NODE_SIZE))
						{
							merr = aern_protocol_error_none;
						}
						else
...
aern_protocol_errors aern_network_register_update_request(aern_network_register_update_request_state* state)
{
	...
	if (state != NULL)
	{
		...
		if (aern_network_connect_to_device(&csock, state->address, aern_network_designation_adc) == qsc_socket_exception_success)
		{
			...
			/* create the join request packet  */
			reqt.pmessage = sbuf + AERN_PACKET_HEADER_SIZE;
			merr = network_register_update_request_packet(&reqt, state);
...
aern_protocol_errors network_register_update_response_packet(aern_network_packet* packetout, aern_network_register_update_response_state* state, uint8_t* buffer, const aern_network_packet* packetin)
{
	...
	if (merr == aern_protocol_error_none)
	{
		...
		if (state->rcert->designation == aern_network_designation_client ||
			state->rcert->designation == aern_network_designation_aps)
		{
			aern_child_certificate ccert = { 0 };
			...
			if (merr == aern_protocol_error_none)
			{
				...
				if (buffer != NULL)
				{
					...
					/* serialize the ADC certificate and add it to the message */
...
aern_protocol_errors aern_network_register_update_v2_response(aern_network_register_update_v2_response_state* state, const aern_network_packet* packetin)
{
	AERN_ASSERT(state != NULL);
	AERN_ASSERT(packetin != NULL);
	...
	if (state != NULL && packetin != NULL && state->csock != NULL && state->lcert != NULL && state->rcert != NULL && state->root != NULL && state->sigkey != NULL && state->vtopo != NULL)
	{
		...
		if (merr == aern_protocol_error_none)
		{
			...
			ncnt = aern_topology_list_server_count(state->vtopo, aern_network_designation_aps);
			mlen = NETWORK_JOIN_UPDATE_RESPONSE_PACKET_SIZE + sizeof(uint64_t);
			mlen += (ncnt * AERN_NETWORK_TOPOLOGY_NODE_SIZE);
			pbuf = (uint8_t*)qsc_memutils_malloc(mlen);

			if (pbuf != NULL)
			{
				mlen -= AERN_PACKET_HEADER_SIZE;
				network_header_create(&resp, aern_network_flag_register_update_response, NETWORK_JOIN_UPDATE_RESPONSE_SEQUENCE, (uint32_t)mlen);
				resp.pmessage = pbuf + AERN_PACKET_HEADER_SIZE;

				network_subheader_serialize(resp.pmessage, &resp);
				mpos = AERN_PACKET_SUBHEADER_SIZE;

				aern_certificate_child_serialize(resp.pmessage + mpos, state->lcert);
				mpos += AERN_CERTIFICATE_CHILD_SIZE;

				qsc_intutils_le64to8(resp.pmessage + mpos, state->vtopo->version);
				mpos += sizeof(uint64_t);

				mlen = aern_topology_list_update_pack(resp.pmessage + mpos, state->vtopo, aern_network_designation_aps);
				mpos += mlen;

				mlen = aern_certificate_message_hash_sign(resp.pmessage + mpos, state->sigkey, resp.pmessage, mpos);
...
aern_protocol_errors aern_ars_remote_signing_handle(aern_ars_signing_state* state, const aern_network_packet* packetin)
{
	AERN_ASSERT(state != NULL);
	AERN_ASSERT(packetin != NULL);
	...
	if (state != NULL && packetin != NULL && state->csock != NULL && state->dcert != NULL && state->root != NULL && state->sigkey != NULL)
	{
		aern_child_certificate rcert = { 0 };
		aern_network_remote_signing_response_state rsr = {
			.csock = state->csock,
			.dcert = (aern_child_certificate*)state->dcert,
			.rcert = &rcert,
			.root = state->root,
			.sigkey = state->sigkey
		};
		...
		qsc_memutils_secure_erase(&rcert, sizeof(rcert));
	}
	else
	{
		merr = aern_protocol_error_invalid_request;
...
* The broadcast revocation of a certificate by the ADC.
...
* Processes the broadcast revocation of a certificate by the ADC.
...
aern_protocol_errors aern_network_revoke_response(aern_network_revoke_response_state* state, const aern_network_packet* packetin)
{
	...
	if (state != NULL && packetin != NULL && state->list != NULL && state->rnode != NULL && state->dcert != NULL)
	{
		/* verify the packet */
		...
		{
			/* find the node in the topological list */
			if (aern_topology_node_find(state->list, state->rnode, ser) == false)
			{
				merr = aern_protocol_error_node_not_found;
			}
		}
	...
	qsc_memutils_secure_erase(ser, sizeof(ser));
...
aern_network_designations aern_network_port_to_application(uint16_t port)
{
	...
	{
		tnode = aern_network_designation_adc;
	}
...
uint16_t aern_network_application_to_port(aern_network_designations tnode)
{
	...
	else if (tnode == aern_network_designation_adc)
	{
